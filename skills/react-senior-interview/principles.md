...
> **Canonical source.** This catalog lives in `react-senior-code-review/`. `react-senior-interview/` carries a **synced copy** so both skills work independently — after editing here, run `scripts/sync-references.sh` from the repo root to propagate.

Read by:

- **`react-senior-code-review`** — for code reviews (find violations, propose fixes).
- **`react-senior-interview`** — for self-assessment quizzes (turn principles into MCQs).
...
| **Severity rubric** | [references/severity-rubric.md](./references/severity-rubric.md) | CRITICAL → NIT definitions and caps (used by `react-senior-code-review`) |
